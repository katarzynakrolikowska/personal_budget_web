...
function setModalOfIncomeEditionAndExpenseEdition()
{
	...
	$('.js-link-edit-Income').on('click', function() {
		...
		//do edycji przychodu
		//var sumRowId = $previousGeneralisedRow.find('.nowrap').id;
...
{
	//klasa balanceEditForm potrzeban do edycji - wstawić w pliku balanceDisplaying.php
		...
		$.post('index.php?action=' + action, details, function(data) {
			if (data.success) {
				...
				if (data.modal) {
					...
					//id #rowResultBg i #colResultBg należy wstawić w pliku balanceDisplaing.php
	...
	$form.on('submit', function(e) {
		...
		$.ajax({
			...
			success: function(data) { 
				...
				$('#usernameEditionModal').modal('hide');
				$('#modalActionResult').modal('show');
