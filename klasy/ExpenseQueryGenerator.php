...
class ExpenseQueryGenerator extends QueryGenerator
{
    ...
    public function getDetailedExpensesOfSelectedCategoryAndPeriod($category, $startDate, $endDate)
    {
        ...
        WHERE e.user_id=:userID 
            AND e.expense_category_assigned_to_user_id = (SELECT ecu.id FROM expenses_category_assigned_to_users as ecu WHERE ecu.name=:category AND ecu.user_id=e.user_id) 
            AND e.payment_method_assigned_to_user_id = pmu.id 
            AND pmu.user_id = e.user_id
            AND e.date_of_expense>=:startDate AND 
            e.date_of_expense<=:endDate 
    ...
    {
        $query = 'UPDATE expenses SET expense_category_assigned_to_user_id = :categoryID, payment_method_assigned_to_user_id = :paymentMethodId, amount = :amount, date_of_expense = :date, expense_comment = :comment WHERE id = :expenseID';
